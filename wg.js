...
// Countries list used for WG menu paging (limited to Germany only)
const WG_COUNTRIES = ['DE'];
...
export async function handleWireguardCallback(data, ctx) {
  ...
  if (data.startsWith('PS:WGCONF:')) {
    ...
    const settings = await getSettings(env);
    const cost = settings.cost_wg || 2;
    if ((user.diamonds || 0) < cost) { await tgApi('sendMessage', { chat_id: chatId, text: `⚠️ سکه کافی نیست. این سرویس ${cost} سکه هزینه دارد.` }); return; }
